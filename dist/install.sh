...
{ lspci -n 2>/dev/null | grep -q "14e4:" && DRIVERS+=" broadcom-wl" || true; }
{ lspci -n 2>/dev/null | grep -q "10de:" && DRIVERS+=" nvidia nvidia-utils nvidia-settings" || true; }
...
sudo systemctl daemon-reload && sudo usermod -aG video,audio,lp,scanner $USER >/dev/null 2>&1
